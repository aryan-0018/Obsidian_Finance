...
const PageHeader = ({ title, subtitle, rightAction, renderPageHeader }: PageHeaderProps) => {
  return (
    <div className="w-full pb-20 pt-4 px-5 lg:px-0 bg-[#0f0f0f] text-[#e8e0d4] border-b border-[#2a2520]">
            ...
            <div className="w-full flex flex-col gap-3 items-start justify-start lg:items-center lg:flex-row lg:justify-between">
              {(title || subtitle) && (
                <div className="space-y-1">
                  {title && <h2 className="text-2xl lg:text-4xl font-medium text-[#e8e0d4]">{title}</h2>}
                  {subtitle && <p className="text-[#9a9080] text-sm">{subtitle}</p>}
                </div>
              )}
